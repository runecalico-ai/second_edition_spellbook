...
## Purpose
This specification defines local LLM chat: model provisioning and loading, the FTS5-based RAG pipeline, the streaming Tauri API, and the Chat UI. All inference runs in Rust via `llama-cpp-rs`; the Python sidecar is not used.

## ADDED Requirements

### Requirement: Model Provisioning
The application SHALL provision the approved TinyLlama 1.1B Q4_K_M GGUF file at `SpellbookVault/models/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf` by explicit in-app download or verified side-load. Normal operation SHALL remain offline after provisioning.

#### Scenario: First-time download trigger
- **WHEN** the user opens the Chat panel for the first time (model not present)
- **THEN** the application SHALL show a provisioning prompt with the model size (~700 MB), a "Download Model" button, and an "Add Local Model" action
...
- **THEN** the backend SHALL emit `llm://download-progress` events with `{ bytesDownloaded: u64, totalBytes: u64 }` payloads
...
- **THEN** the application SHALL surface an error message that includes the failure reason
...
- **WHEN** the user chooses "Add Local Model" and selects the exact approved TinyLlama file
- **THEN** the backend SHALL verify the expected file identity and SHA-256 hash
- **AND** SHALL copy the file into `SpellbookVault/models/`
- **AND** SHALL transition the model status to `ready`

#### Scenario: Side-load rejection
- **WHEN** the user selects a local file that does not match the approved file identity or SHA-256 hash
- **THEN** the backend SHALL reject the file
- **AND** SHALL keep the model status unchanged
- **AND** SHALL return a validation error

### Requirement: Download and Generation Cancellation
The application SHALL expose separate cancellation flows for model downloads and active generation.

#### Scenario: Download cancellation
- **WHEN** a model download is in progress and the user cancels it
- **THEN** the backend SHALL stop the active download
- **AND** SHALL transition model status back to `notProvisioned`
- **AND** SHALL preserve partial bytes for a later resume attempt

#### Scenario: Generation cancellation
- **WHEN** a streamed response is in progress and the user cancels generation
- **THEN** the backend SHALL stop the inference loop on the next token boundary
- **AND** SHALL keep the partial response visible in the chat
- **AND** SHALL NOT affect the model download state
...
- **THEN** it SHALL return one of: `notProvisioned`, `downloading`, `ready`, `loaded`, or `error`

#### Scenario: State transitions
- **WHEN** the model progresses through its lifecycle
- **THEN** the following state transitions SHALL be valid: `notProvisioned` → `downloading` → `ready` → `loaded`; `notProvisioned` → `ready` (on successful side-load); `downloading` → `notProvisioned` (on cancellation); `ready` → `error` (on RAM check failure during load); `loaded` → `error` (on inference initialization failure)
- **AND** transitioning from `error` back to `ready` SHALL require the user to retry provisioning or loading explicitly

### Requirement: Model Loading
The application SHALL load the model on the first chat request and keep it in memory for the rest of the session.
...
#### Scenario: Chat without embedding model
- **WHEN** the embedding model is not provisioned, still initializing, or in a failed state
- **THEN** `llm_chat` SHALL remain available
- **AND** the chat response SHALL continue to use FTS5-only retrieval for grounding

### Requirement: RAG Retrieval Pipeline
Before each response, the backend SHALL extract search terms and retrieve relevant spells to ground the response.
...
- **THEN** the backend SHALL extract 1–3 search terms with a heuristic that strips stop words and retains domain nouns and adjectives
...
- **THEN** the backend SHALL proceed without RAG context
...
- **THEN** the backend SHALL emit a final `llm://done/<stream_id>` event with payload `{ fullResponse: String }`
...
#### Scenario: Empty state (model not provisioned)
- **WHEN** the user opens the Chat panel and the model is not provisioned
- **THEN** the panel SHALL display a feature explanation, the model size, a "Download Model" button, and an "Add Local Model" action
...
- **AND** clicking a spell link SHALL navigate to the existing spell editor route for that spell
