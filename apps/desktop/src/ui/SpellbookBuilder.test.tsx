  ...
  describe("failure notifications", () => {
    ...
    it("add failure shows notification-viewport error toast and does not call window.alert", async () => {
      ...
      await waitFor(() => {
        ...
        const toast = within(viewport).getByTestId("toast-notification-error");
        expect(toast.textContent).toMatch(/Failed to add spell: ipc failed/);
      });
    ...
    it("remove failure shows notification-viewport error toast and does not call window.alert", async () => {
      ...
      await waitFor(() => {
        const viewport = screen.getByTestId("notification-viewport");
        const toast = within(viewport).getByTestId("toast-notification-error");
        expect(toast.textContent).toMatch(/Failed to remove spell: delete failed/);
      });
