    ...
    <div className='space-y-6'>
      <header className='flex items-center justify-between'>
        ...
        <div className='flex items-center gap-2'>
          <Link to='/' className='px-3 py-2 bg-neutral-800 rounded-md text-sm'>Back to Library</Link>
      ...
      <section className='grid md:grid-cols-2 gap-4'>
        <div className='space-y-4'>
          <div>
            ...
            <input
              className={clsx('w-full bg-neutral-900 border rounded-md px-3 py-2', validationErrors.name ? 'border-red-500' : 'border-neutral-700')}
              value={form.name}
          ...
          <div>
            ...
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.school} onChange={(e) => updateField('school', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Sphere</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.sphere} onChange={(e) => updateField('sphere', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Class List (JSON)</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.class_list} onChange={(e) => updateField('class_list', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Level</label>
            <input
              type='number'
              className={clsx('w-full bg-neutral-900 border rounded-md px-3 py-2', validationErrors.level ? 'border-red-500' : 'border-neutral-700')}
          ...
          <div>
            ...
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.range} onChange={(e) => updateField('range', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Components</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.components} onChange={(e) => updateField('components', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Material Components</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.material_components} onChange={(e) => updateField('material_components', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Casting Time</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.casting_time} onChange={(e) => updateField('casting_time', e.target.value)} />
          </div>
        </div>
        <div className='space-y-4'>
          <div>
            <label className='text-sm text-neutral-400'>Duration</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.duration} onChange={(e) => updateField('duration', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Area</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.area} onChange={(e) => updateField('area', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Saving Throw</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.saving_throw} onChange={(e) => updateField('saving_throw', e.target.value)} />
          </div>
          <div className='flex items-center gap-2'>
            <input id='reversible' type='checkbox' checked={form.reversible} onChange={(e) => updateField('reversible', e.target.checked)} />
            <label htmlFor='reversible' className='text-sm text-neutral-300'>Reversible</label>
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Tags (JSON)</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.tags} onChange={(e) => updateField('tags', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Source</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.source} onChange={(e) => updateField('source', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Edition</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.edition} onChange={(e) => updateField('edition', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>Author</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.author} onChange={(e) => updateField('author', e.target.value)} />
          </div>
          <div>
            <label className='text-sm text-neutral-400'>License</label>
            <input className='w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2' value={form.license} onChange={(e) => updateField('license', e.target.value)} />
          </div>
        </div>
      </section>
      ...
      <section className='space-y-2'>
        ...
        <textarea
          className={clsx('w-full bg-neutral-900 border rounded-md px-3 py-2 min-h-[160px]', validationErrors.description ? 'border-red-500' : 'border-neutral-700')}
      ...
      <section className='grid md:grid-cols-3 gap-4'>
            ...
            history.length ? (
              <div className='space-y-2'>
                {history.map((entry, index) => (
                  <div key={`${entry.changed_at}-${index}`} className='border border-neutral-800 rounded-md p-3 text-sm'>
        ...
        <div className='space-y-2'>
          ...
          <div className='text-xs text-neutral-500'>Use JSON strings for tags and class lists until structured editors are added.</div>
          {timestamps.created_at && (
            <div className='text-xs text-neutral-500'>Created: {timestamps.created_at}</div>
          )}
          {timestamps.updated_at && (
            <div className='text-xs text-neutral-500'>Updated: {timestamps.updated_at}</div>
          )}
        </div>
      </section>
    </div>
