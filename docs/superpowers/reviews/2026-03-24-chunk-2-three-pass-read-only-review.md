...
## Resolution Status (2026-03-24)

| ID | Status | Notes |
|----|--------|-------|
| H-001 | ✅ RESOLVED | Fixed 2026-03-24 |
| H-002 | ✅ RESOLVED | Fixed 2026-03-24 |
| M-001 | ⏳ CANNOT AUTO-RESOLVE | Human-only: requires NVDA on Windows/Chromium |
| M-002 | ✅ RESOLVED | Fixed 2026-03-24 |
| M-003 | ✅ RESOLVED | Fixed 2026-03-24 |
| L-001 | — | Out of scope (Low) |
...
### High
[H-001] (71) — Hidden wall-width validation can block saves with no visible recovery path ✅ RESOLVED 2026-03-24
...
Fix: Removed `"wall"` from the `width` entry's `kinds` array in `AREA_SCALAR_FIELDS` (`spellEditorValidation.ts`). Wall areas now only validate the three scalar fields that the wall editor UI actually renders (length, height, thickness). A regression-guard test was added to confirm wall areas never produce width errors.

[H-002] (63) — Dependent validation is not revealed immediately for non-tradition controllers ✅ RESOLVED 2026-03-24
...
Fix: All four non-tradition controlling fields now call `revealFieldValidation` with their dependent fields on every `onChange`:
- `level` onChange → reveals `spell-level`, `spell-school`, `spell-sphere`, `spell-classes`
- `isCantrip` onChange → reveals `spell-level`, `spell-school`, `spell-sphere`, `spell-classes`
- `isQuestSpell` onChange → reveals `spell-level`, `spell-sphere`, `spell-classes`
- `classList` onChange → reveals `spell-classes` (also now has an onChange reveal, not only onBlur)

### Medium
[M-001] (42) — Required NVDA acceptance evidence is still missing from the plan artifact ⏳ CANNOT AUTO-RESOLVE
...
Status: Requires a human to launch NVDA on Windows/Chromium, exercise the validation paths, and record verbatim announcements. No automated tooling can produce or substitute for this evidence.

[M-002] (37) — Casting-time inputs set `aria-invalid="false"` instead of omitting the attribute when valid ✅ RESOLVED 2026-03-24
...
Fix: Both casting-time inputs in `StructuredFieldInput.tsx` changed from `aria-invalid={err ? "true" : "false"}` to `aria-invalid={err ? "true" : undefined}`, so the attribute is absent (not `"false"`) in the valid state. This matches the WAI-ARIA spec and the pattern used by all other editor inputs.

[M-003] (28) — Most explicit AreaForm scalar variants are left unproven by tests ✅ RESOLVED 2026-03-24
...
Fix: Added 9 tests to `spellEditorValidation.test.ts` covering every previously untested `AREA_SCALAR_FIELDS` mapping: `width` (rect, fixed), `width` (rect_prism, per-level), `height` (rect_prism), `thickness` (wall), `edge` (cube), `surfaceArea` (surface), `volume` (volume), `tileCount` (tiles), `count` (creatures). Includes a dedicated H-001 regression guard confirming wall areas produce zero width errors.
