...
test.describe("Character Profile Remediation", () => {
  test("should handle 'Other' class with custom label", async ({ appContext }) => {
  ...
  test("should enforce 'Prepared MUST be Known' in Spell Picker", async ({ appContext }) => {
    const { page } = appContext;
    const app = new SpellbookApp(page);
    const runId = generateRunId();
  ...
  test("should delete character from profile header", async ({ appContext }) => {
    const { page } = appContext;
    const app = new SpellbookApp(page);
    const runId = generateRunId();
