...
- **Metadata Exclusion**: `source_refs`, `edition`, `author`, `version`, `license`, and `schema_version` are strictly excluded from the canonical JSON used for hashing, but are **preserved** in standard serialization for storage and export.
