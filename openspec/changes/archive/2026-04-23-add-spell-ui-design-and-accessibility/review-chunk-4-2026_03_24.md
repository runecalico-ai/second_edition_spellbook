...
Post-fix status as of 2026-03-24:

- Resolved: `C-001`, `C-002`, `H-001`, `H-002`, `H-003`, `M-001`, `M-002`, `M-003`, `M-004`, `M-005`, `M-006`
- Remaining Low: `L-001`, `L-002`, `L-003`, `L-004`, `L-005`
- Verification evidence:
	- `pnpm exec vitest run src/ui/components/structured/StructuredFieldInput.test.tsx src/ui/components/structured/ComponentCheckboxes.test.tsx src/ui/components/structured/SavingThrowInput.test.tsx src/ui/components/structured/DamageForm.test.tsx src/ui/components/structured/MagicResistanceInput.test.tsx --project=unit` → 5 files passed, 61 tests passed
	- `pnpm exec vitest run --project=storybook src/ui/components/structured/StructuredFieldInput.stories.tsx src/ui/components/structured/ComponentCheckboxes.stories.tsx` → 2 files passed, 34 tests passed
...
**[C-001] (92) — Three structured editor components have no light-mode theming — RESOLVED**
...
Resolution: Added dual light/dark surface, border, muted-text, and annotation tokens to the three affected components and covered the updated contract with targeted unit tests.

**[C-002] (88) — Three structured editor components have zero test coverage — RESOLVED**
...
Resolution: Added dedicated unit coverage for `SavingThrowInput`, `DamageForm`, and `MagicResistanceInput` covering grouped DOM structure, theming hooks, annotations, and `onChange` behavior.

---

### High

**[H-001] (68) — `bg-neutral-950` used as a dark-mode background throughout — not in the palette — RESOLVED**
...
Resolution: Replaced the reviewed `neutral-950` dark-surface tokens in production structured components with palette-aligned `dark:bg-neutral-800` and `dark:bg-neutral-700` roles and updated the associated tests.

**[H-002] (62) — `border-neutral-500` on interactive inputs — not a named palette border role — RESOLVED**
...
Resolution: Normalized reviewed interactive inputs to `border-neutral-400` and softened grouped container borders to `border-neutral-300` where appropriate, preserving existing dark-mode border roles.

**[H-003] (55) — No dark-mode story decorator in StructuredFieldInput or ComponentCheckboxes Storybook stories — RESOLVED**
...
Resolution: Added dark-theme Storybook decorators/stories for both files and added explicit `all`-variant coverage for `ComponentCheckboxes`.

---

### Medium

**[M-001] (47) — ComponentCheckboxes `<output>` preview missing `aria-label` — RESOLVED**
...
Resolution: Added `aria-label="Computed component text"` to the preview `<output>` and locked it with a unit test.

**[M-002] (44) — AreaForm preview uses `<p>` instead of `<output>` — inconsistent semantic contract — RESOLVED**
...
Resolution: Replaced the preview paragraph with `<output aria-label="Computed area text">` while leaving the existing low-priority contrast issue unchanged.

**[M-003] (42) — Label placement convention not codified within structured group components — RESOLVED**
Plan ref: tasks.md Chunk 4 library: "Define label placement and container treatment for structured field groups."
...
Resolution: Codified grouped semantics with internal `fieldset` + `sr-only` `legend` labels across the reviewed structured group components and added tests for the contract.

**[M-004] (38) — StructuredFieldInput tests missing light-mode class assertions — RESOLVED**
...
Resolution: Updated the structured-field unit tests to assert the corrected light-mode root, supporting-row, and preview-row classes.

**[M-005] (35) — ComponentCheckboxes tests missing light-mode class assertions and "all" variant preview content — RESOLVED**
Plan ref: tasks.md Chunk 4 library: "Define `ComponentCheckboxes` … preview treatment."
...
Resolution: Expanded `ComponentCheckboxes` tests to cover the light-mode surface classes, the preview `<output>` contract, and the `all`-variant preview abbreviations.

**[M-006] (29) — ComponentCheckboxes checkbox strip uses `gap-4` while all structured primary rows use `gap-2` — RESOLVED**
...
Resolution: Aligned the checkbox strip to `gap-2` and added a focused unit test so the spacing contract matches the rest of the structured editor rows.
