...
**Non-Goals:**
- Completely rewriting the Python extraction logic (we are merely saving the legacy string during the existing pipeline).
- Altering any logic related to the core combat damage roller itself, beyond the enum changes.
...
- *Rationale:* Frontends should not be responsible for rebuilding legible strings from complex algebraic nodes. By moving `text` alongside the algebraic parts, React components can bind directly to the canonical string. If a user edits an algebraic field, the `text` field will simply be cleared, deferring to the backend or user to regenerate it.

**Decision 3: Deprecation of `dm_guidance` in SavingThrowSpec**
- *Rationale:* `SavingThrowSpec` already contains a general-purpose `notes` array that serves the exact same narrative function for edge cases.

**Decision 3a: Clarification of `save_type` vs `save_vs` Semantics (No Enum Changes)**
...
**Decision 4: Schema Version Bump to 2**
...
- *Implementation:* A `migrate_to_v2()` function is added to the `normalize()` pipeline. When a spell with `schema_version < 2` is encountered, the migration performs:
  ...
  2. **CastingTime 5e unit mapping**: If `casting_time.unit` is `"action"`, `"bonus_action"`, or `"reaction"`, remap to `"special"` and preserve the original `casting_time.text` in `raw_legacy_value` if not already populated.
...
**Decision 5: Normalization Modes for New Fields**
...
- *New field normalization modes:*
  ...
  - `DurationSpec.text` → **Structured + unit alias normalization** (word boundaries). Same treatment as `RangeSpec.text` and `AreaSpec.text`. Duration text may reference distance units in rare compound specifications, and uniform unit alias normalization prevents hash drift.
...
**Decision 6: Resolved Specs Do Not Include `text` or `raw_legacy_value`**
...
- *Risk:* React components binding to `AreaSpec` and `DurationSpec` might get confused if a user alters the algebraic portion (e.g., radius: 10 to 20) but the `text` field still awkwardly says "10 ft. radius".
- *Mitigation:* The React UI bindings MUST automatically clear the `text` override field if any of the underlying dimension components are modified by the user.
...
4. Implement `migrate_to_v2()` in the `normalize()` pipeline:
   - Move `SavingThrowSpec.dm_guidance` content into `notes` (append with newline separator if `notes` already populated).
   - Remap 5e `casting_time.unit` values (`"action"`, `"bonus_action"`, `"reaction"`) to `"special"`, preserving original text in `raw_legacy_value`.
...
6. Update `docs/architecture/canonical-serialization.md`:
   ...
   - Normalization table: Add entries with explicit modes per Decision 5:
...
## Open Questions
- None at this time. *(Normalization modes, `raw_legacy_value` patterns, and resolved spec scope clarified in Decisions 5 and 6.)*
