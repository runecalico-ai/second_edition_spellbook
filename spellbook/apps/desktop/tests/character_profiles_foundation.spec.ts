...
test.describe("Character Profiles Foundation", () => {
    ...
    test("should handle multi-classing and per-class spell lists", async () => {
        ...
        await app.createSpell({ name: mageSpell, level: "1", school: "Evocation", description: "Mage spell description" });
        await app.createSpell({ name: clericSpell, level: "1", sphere: "All", description: "Cleric spell description" });
    ...
    test("should enforce Known spell requirement for Prepared list", async () => {
        ...
        await app.createSpell({ name: testSpell, level: "1", school: "Alteration", description: "Test" });
        ...
        const spellRow = mageSection.getByText(testSpell).locator('..').locator('..').locator('..').locator('..');
        // Force click to bypass potential visibility/overlap issues
        await spellRow.locator('button').filter({ has: page.locator('svg path[d="M3 6h18"]') }).click({ force: true });
