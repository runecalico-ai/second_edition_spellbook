...
test.describe("Milestone Verification Flow", () => {
	...
	test("Milestone 2: Import Wizard & Provenance", async ({
		...
		const samplePath = fileTracker.track(
			path.resolve(__dirname, `tmp/sample-${runId}.md`),
		);
		fs.writeFileSync(
			samplePath,
		...
		await test.step("Import a markdown file", async () => {
			...
			await expect(
				page.getByRole("button", { name: "Import More Files" }),
			).toBeVisible({
...
test("Import conflict merge review flow", async ({
	...
	await test.step("Trigger conflict import and resolve", async () => {
		const samplePath = fileTracker.track(
			path.resolve(__dirname, `tmp/conflict-${runId}.md`),
		);
		fs.writeFileSync(
			samplePath,
			`---\nname: ${conflictName}\nlevel: 1\nsource: ${conflictSource}\n---\n${incomingDescription}`,
		...
		await expect(
			page.getByRole("button", { name: "Custom Merge" }),
		).toBeVisible();
		await expect(
			page.getByRole("button", { name: "Use Incoming" }),
		).toBeVisible();
		await expect(
			page.getByRole("button", { name: "Keep Existing" }),
		).toBeVisible();
		...
		await expect(
			page.getByRole("columnheader", { name: "Incoming" }),
		).toBeVisible();
...
test("Spell editor persists extended fields", async ({
	...
	const importedPath = fileTracker.track(
		path.resolve(__dirname, `tmp/extended-${runId}.md`),
	);
	fs.writeFileSync(
		importedPath,
		[
	...
	await test.step("Load existing spell and confirm fields populate", async () => {
		...
		await expect(page.getByLabel("Classes (e.g. Mage, Cleric)")).toHaveValue(
			"Mage",
		);
		...
		await expect(page.getByPlaceholder("Components (V,S,M)")).toHaveValue(
			"V,S,M",
		);
		...
		await expect(page.getByLabel("Material Components")).toHaveValue(
			"a crystal lens",
		);
