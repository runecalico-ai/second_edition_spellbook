...
const TAURI_BIN = (() => {
    ...
    const targetPath = path.resolve(__dirname, "../src-tauri/target/x86_64-pc-windows-msvc/debug/spellbook-desktop.exe");
...
test("Milestone 3: Robust Search & Saved Searches", async () => {
    ...
    await page.getByRole("combobox").filter({ hasText: "Saved Searches" }).selectOption({ label: saveName });
    await page.getByRole("button", { name: "Search", exact: true }).click();
    await expect(page.getByRole("row", { name: level5Spell })).toBeVisible();
    await expect(page.getByRole("row", { name: spellName })).not.toBeVisible();
