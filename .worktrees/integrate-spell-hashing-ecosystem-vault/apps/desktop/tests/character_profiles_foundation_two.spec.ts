...
test.describe("Character Profiles Foundation", () => {
  test("should handle character creation, identity, and abilities", async ({ appContext }) => {
  ...
  test("should delete a character including checking cascade", async ({ appContext }) => {
    const { page } = appContext;
    const app = new SpellbookApp(page);
