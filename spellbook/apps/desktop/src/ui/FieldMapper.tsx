...
export default function FieldMapper({ spells, onConfirm, onCancel }: FieldMapperProps) {
    ...
    const updateField = (field: keyof ParsedSpell, value: string | number) => {
        setEditedSpells((prev) =>
            prev.map((spell, i) =>
                i === currentIndex ? { ...spell, [field]: value } : spell
            )
        );
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="text-xs text-neutral-500 truncate">
                Source: {currentSpell._source_file}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label htmlFor="fieldmapper-name" className="text-sm text-neutral-400">Name *</label>
                        <span className={`text-[10px] px-1 rounded ${getConfidenceBadge(getFieldConfidence("name"))}`}>
                ...
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label htmlFor="fieldmapper-level" className="text-sm text-neutral-400">Level *</label>
                        <span className={`text-[10px] px-1 rounded ${getConfidenceBadge(getFieldConfidence("level"))}`}>
                ...
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label htmlFor="fieldmapper-school" className="text-sm text-neutral-400">School</label>
                        <span className={`text-[10px] px-1 rounded ${getConfidenceBadge(getFieldConfidence("school"))}`}>
                ...
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label htmlFor="fieldmapper-source" className="text-sm text-neutral-400">Source</label>
                        <span className={`text-[10px] px-1 rounded ${getConfidenceBadge(getFieldConfidence("source"))}`}>
            ...
            <div>
                <div className="flex justify-between items-center mb-1">
                    <label htmlFor="fieldmapper-description" className="text-sm text-neutral-400">Description *</label>
                    <span className={`text-[10px] px-1 rounded ${getConfidenceBadge(getFieldConfidence("description"))}`}>
            ...
            <div className="flex justify-between pt-4 border-t border-neutral-800">
                ...
                <div className="flex gap-1">
                    {Array.from({ length: Math.min(totalSpells, 10) }, (_, i) => (
                        <button
                            key={`page-${i}`}
                            type="button"
                            onClick={() => setCurrentIndex(i)}
                            className={`w-6 h-6 text-xs rounded ${i === currentIndex
