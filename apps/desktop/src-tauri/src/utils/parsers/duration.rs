...
impl DurationParser {
    ...
    pub fn parse(&self, input: &str) -> DurationSpec {
        ...
        // Per task 1.4: empty input must produce None, not Some("").
        res.raw_legacy_value = if input_clean.is_empty() {
            None
        } else {
...
mod tests {
    ...
    fn test_unconditional_legacy_text_preservation() {
        ...
        // Success case
        let res = parser.parse("10 rounds");
        assert_eq!(res.raw_legacy_value.as_ref().unwrap(), "10 rounds");

        // Fallback case
        let res2 = parser.parse("Unknown duration");
        assert_eq!(res2.kind, DurationKind::Special);
        assert_eq!(res2.raw_legacy_value.as_ref().unwrap(), "Unknown duration");
    }

    /// Task 1.5: Empty input must yield raw_legacy_value: None, never Some("").
    #[test]
    fn test_parse_duration_empty_raw_legacy_value_none() {
        let parser = DurationParser::new();
        let res = parser.parse("");
        assert!(
            res.raw_legacy_value.is_none(),
            "empty input must produce raw_legacy_value None, got {:?}",
            res.raw_legacy_value
        );
        let res_ws = parser.parse("   ");
        assert!(
            res_ws.raw_legacy_value.is_none(),
            "whitespace-only input must produce raw_legacy_value None"
        );
    }
}
