...
export default function Chat() {
  ...
  return (
    <div className="space-y-3">
      ...
      />
      <button className="px-3 py-2 bg-neutral-800 rounded-md" onClick={ask} type="button">
        Ask
      </button>
      <div className="bg-neutral-900 border border-neutral-800 rounded-md p-3 text-sm whitespace-pre-wrap">
        {a}
      </div>
      {citations.length > 0 && (
        <div className="text-xs text-neutral-400">Citations: {citations.join(", ")}</div>
      )}
    </div>
  );
}
