...
test("Epic and Quest Spells E2E (Connected)", async () => {
    ...
    await page.waitForLoadState("networkidle", { timeout: 30000 }).catch(e => console.log(`Wait failed: ${e}`));
    ...
    await expect(page.getByRole("heading", { name: "New Spell", exact: true })).toBeVisible({ timeout: 15000 });
